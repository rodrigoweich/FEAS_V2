...
<div class="d-flex" id="wrapper">
    ...
    <div class="bg-light border-right" id="sidebar-wrapper">
        <div class="list-group list-group-flush">
            ...
            <div id="divider">
                <a id="navbarDropdown" class="nav-link dropdown-toggle list-group-item list-group-item-action bg-light" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                    <span class="float-right"><i class="fas fa-minus"></i></span> <span id="mapTypesDpMenu">Cables</span> <span class="caret"></span>
                </a>

                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
                    @foreach($cables as $cable)
                    <a class="dropdown-item" onclick="selectCableType({{ $cable->size }}, '{{ $cable->color }}', {{ $cable->dotted }}, {{ $cable->dotted_repeat }})">
            ...
            <div id="divider">
                <a id="navbarDropdown" class="nav-link dropdown-toggle list-group-item list-group-item-action bg-light" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                    <span class="float-right"><i id="figureType" class="fas fa-home"></i></span> <span id="mapTypesDpMenu">Figure Option</span> <span class="caret"></span>
                </a>

                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="navbarDropdown">
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-home')">
                        {{ __('Home') }}<span class="float-right"><i class="fas fa-home"></i></span>
                    </a>
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-hotel')">
                        {{ __('Hotel') }}<span class="float-right"><i class="fas fa-hotel"></i></span>
                    </a>
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-building')">
                        {{ __('Building') }}<span class="float-right"><i class="fas fa-building"></i></span>
                    </a>
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-hospital')">
                        {{ __('Hospital') }}<span class="float-right"><i class="fas fa-hospital"></i></span>
                    </a>
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-store')">
                        {{ __('Store') }}<span class="float-right"><i class="fas fa-store"></i></span>
                    </a>
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-warehouse')">
                        {{ __('Warehouse') }}<span class="float-right"><i class="fas fa-warehouse"></i></span>
                    </a>
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-church')">
                        {{ __('Church') }}<span class="float-right"><i class="fas fa-church"></i></span>
                    </a>
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-graduation-cap')">
                        {{ __('Graduation') }}<span class="float-right"><i class="fas fa-graduation-cap"></i></span>
                    </a>
                    <a class="dropdown-item" onclick="changeFigureType('fas fa-industry')">
                        {{ __('Industry') }}<span class="float-right"><i class="fas fa-industry"></i></span>
                    </a>
                </div>
            </div>
            <a onclick="createChangeCustomerPointFunction()" class="list-group-item list-group-item-action bg-light">{{ __('Edit Customer Point') }}<span class="float-right"><i class="far fa-edit"></i></span></a>
            <a onclick="createListenerToTheServiceBox(boxesMarkers, boxesIds)" class="list-group-item list-group-item-action bg-light">{{ __('Edit Box Service') }}<span class="float-right"><i class="far fa-edit"></i></span></a>
            <a onclick="showAlertSave()" class="list-group-item list-group-item-action bg-light">{{ __('Save process') }}<span class="float-right"><i class="fas fa-map-marked"></i></span></a>
            <a href="{{ route('default.process_stage_one.index') }}" class="list-group-item list-group-item-action bg-light text-danger">{{ __('Cancel and return') }}<span class="float-right"><i class="far fa-hand-point-left"></i></span></a>
        </div>
    </div>
    <!-- /#sidebar-wrapper -->
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">{{ __('Process information') }}</h5>
            ...
            <div class="modal-body">
                ...
                <form action="{{ route('default.process_stage_two.update', $response) }}" method="post">
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-4">
                                    <label for="complement">{{ __('Address Complement') }}</label>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-2">
                                    ...
                                    <input type="text" min="0" max="30" class="form-control" id="box" name="box" value="{{ $response->customer()->get()->first()->service_boxes_id }}" readonly>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-12">
                                    <label for="note">{{ __('What degree of difficulty does this process represent?') }} <span class="text-danger" id="rangeNumber">3</span></label>
                                    <input type="range" class="custom-range" min="0" max="5" name="note" id="note" onchange="changeNoteNumber(this.value)">
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col-md-12">
                                    <div class="custom-file">
                                        ...
                                        <label class="custom-file-label" for="photos">{{ __('Insert the photos of this activation') }}</label>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <span class="float-right">
                        <a class="btn btn-danger" href="{{ route('default.process_stage_one.index') }}" role="button">{{ __('Cancel and return') }}</a>
...
<div class="modal fade" tabindex="-1" role="dialog" id="alertDeleteElement">
    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">You can only enter a position / construction via feasibility! <br />
                 To change the position / construction you need to delete the previously inserted position / construction. <br />
                 Do you really want to proceed?</p>
...
function loadServiceBoxes(position, availables, boxId) {
...
function createListenerToTheServiceBox(array, arrayId) {
    $.each(array, function(i) {
        array[i].addListener("click", () => {
            ...
            $('#box').val(arrayId[array.indexOf(array[i])]);
...
function loadCablesOnMap() {
    @foreach($cables as $cable)
        @if ($loop->first)
            selectCableType({{ $cable->size }}, '{{ $cable->color }}', {{ $cable->dotted }}, {{ $cable->dotted_repeat }});
        @endif
    @endforeach
};
...
function changeNoteNumber(value) {
    $("#rangeNumber").text();
